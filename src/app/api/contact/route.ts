...
export async function POST(req: NextRequest) {
  try {
    ...
    // Önce Veritabanına Kaydet (Kalıcı olmasını garanti altına al)
    await connectDB();
    await Message.create({ name, email, message });

    // Sonra E-Posta bildirimi gönder (Basit fail-safe yapılabilir)
    try {
      ...
      console.error("Mail gönderilemedi fakat DB'ye kaydedildi:", mailError);
      // Sadece mail gitmediyse yine de başarılı sayıp kullanıcıyı üzmüyoruz, mesaj DB'de güvenli.
    }
